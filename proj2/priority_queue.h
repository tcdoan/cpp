...
template <class Key> class MinPq {
public:
  ...
  bool Contains(int i) {
    if (i < 0 || i >= maxN)
      throw runtime_error("Illegal index argument");
    return qp[i] != -1;
  }
  ...
  void Insert(int i, Key x) {
    if (i < 0 || i >= maxN)
      throw runtime_error("Illegal index argument");
    if (Contains(i))
      throw runtime_error("bad argument. index already in the queue");
    n++;
    qp[i] = n;
    pq[n] = i;
  ...
  void DecreaseKey(int i, Key key) {
    ...
    if (keys[i] <= key)
      throw runtime_error("Calling DecreaseKey() with given argument not "
                          "strictly decrease the key");
  ...
  void IncreaseKey(int i, Key key) {
    ...
    if (keys[i] >= key)
      throw runtime_error("Calling IncreaseKey() with given argument not "
                          "strictly increase the key");
